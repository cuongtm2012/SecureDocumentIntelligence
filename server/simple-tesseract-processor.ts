...
export class SimpleTesseractProcessor {
  ...
  private async processImageSimple(imagePath: string): Promise<{
    ...
    return new Promise((resolve, reject) => {
      // Optimize for Vietnamese ID cards - PSM 6 works best for structured documents
      const isIdCard = imagePath.toLowerCase().includes('cmt') || 
                      imagePath.toLowerCase().includes('id') ||
                      imagePath.toLowerCase().includes('card');

      const attempts = isIdCard ? [
        { lang: 'vie', psm: 6, name: 'Vietnamese Single Block (ID Card Optimized)' },
        { lang: 'vie', psm: 3, name: 'Vietnamese Auto' },
        { lang: 'vie+eng', psm: 6, name: 'Vietnamese+English Single Block' },
        { lang: 'vie', psm: 8, name: 'Vietnamese Single Word' },
        { lang: 'eng', psm: 6, name: 'English Single Block' }
      ] : [
        { lang: 'vie', psm: 3, name: 'Vietnamese Auto' },
        { lang: 'vie+eng', psm: 3, name: 'Vietnamese+English Auto' },
        { lang: 'eng', psm: 3, name: 'English Auto' },
        { lang: 'vie', psm: 6, name: 'Vietnamese Single Block' },
        { lang: 'vie', psm: 8, name: 'Vietnamese Single Word' },
        { lang: 'vie', psm: 13, name: 'Vietnamese Raw Line' }
      ];
