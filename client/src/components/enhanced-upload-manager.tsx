  ...
  const onDrop = useCallback(
    (acceptedFiles: File[], event?: any) => {
      ...
      if (validFiles.length > 0) {
        console.log(
          `Processing ${validFiles.length} new files for upload ${forceReprocess ? "(force reprocess)" : ""}`,
        );
  ...
  const processFile = async (fileId: string) => {
    ...
    try {
      ...
      eventSource = new EventSource(
        `/api/documents/${file.documentId}/progress-stream`,
      );
      ...
      const response = await fetch(
        `/api/documents/${file.documentId}/process`,
        {
          method: "POST",
        },
      );
      ...
      if (result.structuredData) {
        try {
          structuredData =
            typeof result.structuredData === "string"
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <Tabs
            value={activeTab}
            onValueChange={(value) => setActiveTab(value as "images" | "pdfs")}
          >
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-2">
                      💡 Hold Ctrl/Alt while dropping to force reprocess
                      duplicates
                    </p>
                  </div>
                  <div className="flex gap-2">
                    {onBatchUpload && (
                      <Button
                        onClick={() =>
                          onBatchUpload(uploadedFiles.map((f) => f.file))
                        }
                        variant="outline"
                      ...
                      {file.status === "queued" && (
                        <div className="flex gap-1">
                          <Button
                            size="sm"
                            onClick={() =>
                              onFileProcessWithEngine(file.id, "tesseract")
                            }
                            variant="outline"
                              ...
                              <div
                                className={cn(
                                  ...
                                  file.ocrProgress.stage === "converting" &&
                                    "bg-yellow-500",
                                  file.ocrProgress.stage === "extracting" &&
                                    "bg-green-500",
                                  file.ocrProgress.stage === "reconstructing" &&
                                    "bg-purple-500",
                                  file.ocrProgress.stage === "completing" &&
                                    "bg-emerald-500",
                                )}
                              />
                              {file.ocrProgress.currentStep ||
                                file.ocrProgress.stageDescription}
                              {file.ocrProgress.currentPage &&
                                file.ocrProgress.totalPages && (
                                  <span className="text-xs text-gray-500">
                                    (Page {file.ocrProgress.currentPage}/
                                    {file.ocrProgress.totalPages})
                                  </span>
                                )}
                            </>
                    ...
                    <div className="bg-green-50 dark:bg-green-950 p-4 rounded-lg border border-green-200 dark:border-green-800">
                          ...
                          {(() => {
                            try {
                              const structuredData =
                                typeof file.structuredData === "string"
                                  ? JSON.parse(file.structuredData)
                                  : file.structuredData;
                              if (
                                structuredData?.text_reconstruction?.applied
                              ) {
                                return (
                          ...
                          <Button
                            ...
                            onClick={(e) => {
                              ...
                              if (onViewResult) {
                                ...
                                console.error(
                                  "❌ onViewResult handler not provided",
                                );
                              }
                            }}
                          >
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                        <div className="text-center p-2 bg-white dark:bg-gray-800 rounded">
                          <p className="font-bold text-blue-600">
                            {file.result.characterCount ||
                              file.result.extractedText.length}
                          </p>
                      ...
                      <div className="mt-3 pt-2 border-t border-green-200 dark:border-green-700">
                        ...
                        <p className="text-xs text-green-700 dark:text-green-300 line-clamp-2">
                          ...
                          {file.result.extractedText.substring(0, 120)}
