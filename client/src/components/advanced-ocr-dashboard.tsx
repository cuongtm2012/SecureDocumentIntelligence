...
export function AdvancedOCRDashboard() {
  ...
  const processMutation = useMutation({
    ...
    onSuccess: (data, documentId) => {
      ...
      toast({
        ...
        description:
          "OCR processing completed successfully with enhanced progress tracking.",
      });
    },
  ...
  const parallelProcessMutation = useMutation({
    mutationFn: async (documentId: string) => {
      const response = await fetch(
        `/api/documents/${documentId}/process-parallel`,
        {
          method: "POST",
        },
      );
  ...
  const handleFileUpload = async (files: File[], forceReprocess = false) => {
    ...
    console.log(
      `📤 Uploading ${files.length} files${forceReprocess ? " (force reprocess)" : ""}`,
    );
    ...
    try {
      ...
      setUploadedFiles((prev) =>
        prev.map((f) => {
          const matchingResult = uploadResults.find(
            (result: any) =>
              result.originalFile.name === f.name &&
              result.originalFile.size === f.size,
          );

          if (matchingResult) {
            console.log(
              `📄 Mapping file "${f.name}" to document ID ${matchingResult.id}${matchingResult.isDuplicate ? " (duplicate)" : ""}`,
            );
            return {
              ...f,
  ...
  const trackOCRProgress = (documentId: string, fileId: string) => {
    ...
    const connectToProgressStream = () => {
      eventSource = new EventSource(
        `/api/documents/${documentId}/progress-stream`,
      );
          ...
          setUploadedFiles((prev) =>
            prev.map((f) => {
              ...
              if (logData.message?.includes("Starting PDF OCR processing")) {
                ...
                logData.message?.includes("Page") &&
                logData.message?.includes("processed with")
              ) {
                const pageMatch = logData.message.match(/Page (\d+)/);
                const confidenceMatch =
                  logData.message.match(/(\d+)% confidence/);
                if (pageMatch) {
                  ...
                  progress =
                    40 + (pageNumber / (ocrProgress.totalPages || 1)) * 40; // Up to 80%
                }
                ...
                logData.message?.includes(
                  "DeepSeek API processing completed successfully",
                )
              ) {
                ocrProgress = {
                  ...ocrProgress,
  ...
  const handleFileProcess = async (fileId: string) => {
    ...
    console.log(
      `🔄 Processing document ID ${file.documentId} for file "${file.name}"`,
    );
    ...
    const cleanupProgress = trackOCRProgress(
  ...
  const handleFileParallelProcess = async (fileId: string) => {
    ...
    console.log(
      `🔄 Parallel processing document ID ${file.documentId} for file "${file.name}"`,
    );
    ...
    try {
      ...
      const result = await parallelProcessMutation.mutateAsync(
        file.documentId.toString(),
      );
          ...
          f.id === fileId
            ? {
                ...
                result: {
                  ...
                  wordCount: result.document?.extractedText
                    ? result.document.extractedText
                        .split(/\s+/)
                        .filter((word: string) => word.length > 0).length
                    : 0,
                  characterCount: result.document?.extractedText
      ...
      console.error(
        "❌ Parallel processing failed for document ID",
        file.documentId,
        error,
      );
      setUploadedFiles((prev) =>
  ...
  const handleFileProcessWithEngine = async (
    fileId: string,
    engine: "tesseract" | "parallel" | "receipt",
  ) => {
    const file = uploadedFiles.find((f) => f.id === fileId);
    if (!file || !file.documentId) {
      console.error("❌ Cannot process file: missing document ID", file);
    ...
    console.log(
      `🔄 Processing document ID ${file.documentId} with ${engine} engine for file "${file.name}"`,
    );
    ...
    try {
            ...
            ? {
                ...
                result: {
                  extractedText:
                    ...
                    "",
                  confidence:
                    result.document?.confidence || result.confidence || 0,
                  pageCount:
                    result.document?.structuredData?.pageCount ||
                    result.pageCount ||
                    1,
                  wordCount: (
                    result.document?.extractedText ||
      ...
      toast({
        ...
        description:
          messages[engine as keyof typeof messages] ||
          `${engine} processing completed successfully`,
      });
    } catch (error) {
      console.error(
        `❌ ${engine} processing failed for document ID`,
        file.documentId,
        error,
      );
      setUploadedFiles((prev) =>
  ...
  const handleFileCancel = (fileId: string) => {
    setUploadedFiles((prev) =>
      prev.map((f) => (f.id === fileId ? { ...f, status: "queued" } : f)),
    );
  };
  ...
  const handleQuickExport = (file: UploadedFile) => {
    ...
    window.open(
      `/api/documents/${correspondingDocument.id}/export?format=txt`,
      "_blank",
    );
  };
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">
                    {
                      documents.filter(
                        (d: any) => d.processingStatus === "completed",
                      ).length
                    }
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">
                    {
                      documents.filter(
                        (d: any) => d.processingStatus === "processing",
                      ).length
                    }
                  </p>
                </div>
                        ...
                        <p className="text-2xl font-bold text-blue-600">
                          {
                            documents.filter((d: any) => {
                              ...
                              return (
                                docDate >= todayStart && docDate < todayEnd
                              );
                            }).length
                          }
                        </p>
            ...
            <div className="flex flex-col space-y-4">
              <div className="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-4">
                  ...
                  >
                    Page {currentPage} of{" "}
                    {Math.ceil((filteredDocuments?.length || 0) / pageSize)}
                  </Badge>
                </div>
              </div>
                  ...
                  {paginatedDocuments
                    .sort((a: any, b: any) => {
                      const dateA = new Date(
                        a.processingCompletedAt ||
                          a.processedAt ||
                          a.uploadedAt,
                      ).getTime();
                      const dateB = new Date(
                        b.processingCompletedAt ||
                          b.processedAt ||
                          b.uploadedAt,
                      ).getTime();
                      return dateB - dateA;
                              ...
                              {processingDocuments.has(doc.id.toString()) && (
                                <OCRProgressTracker
                                  ...
                                  onComplete={(success, details) => {
                                    console.log(
                                      `📊 Progress completed for document ${doc.id}:`,
                                      { success, details },
                                    );
                                    setProcessingDocuments((prev) => {
                                      const updated = new Set(prev);
                                    ...
                                    if (success) {
                                      setCompletedDocuments(
                                        (prev) =>
                                          new Set(prev.add(doc.id.toString())),
                                      );
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  ...
                                  setSelectedResult({
                                    ...
                                    fileType: doc.mimeType?.includes("pdf")
                                      ? "pdf"
                                      : "image",
                    ...
                    <div className="grid grid-cols-2 gap-4 text-center">
                      ...
                      <div>
                        <p className="text-xl font-bold text-red-600">
                          {
                            documents.filter(
                              (d: any) => d.processingStatus === "failed",
                            ).length
                          }
                        </p>
